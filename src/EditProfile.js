...
function EditProfile() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const updatedUser = await axios.put(`https://6511b6bab8c6ce52b394f19f.mockapi.io/users/${params.id}`, values);
        if (updatedUser) {
  ...
  let getUserData = async () => {
    try {
      const user = await axios.get(`https://6511b6bab8c6ce52b394f19f.mockapi.io/users/${params.id}`);
  ...
  //   try {
  //     const updatedUser = await axios.put(`https://6511b6bab8c6ce52b394f19f.mockapi.io/users/${params.id}`);
  //   } catch (error) {
  //     alert("Error in updating");
  //   }
  // }
